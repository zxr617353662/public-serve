...
function _loadTeachClazz($clazzList, tid) {
    $.ajax({
        "url": getRootPath() + "/admin/clazz/listByTeacher",
        "data": "tid=" + tid,
        "async": false,
        "dataType": "json",
        "success": function (json) {
            ...
            _handleJSON(json, $("#clazz_list"), function (element) {
                ...
                return "<li><input type='checkbox' name='clazzs' value='" + element.id + "'><span>" + element.gradeDO.name + "级" +
...
function save() {
    ...
    $.ajax({
        "url": getRootPath() + "/admin/teacher/addClass",
