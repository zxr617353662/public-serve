$(function () {
    ...
    //获取题目的类型
    QuestionHelper.questionType="[[${type}]]"
    QuestionHelper.questionType =question_type;
...
var QuestionHelper = {
        ...
        if (!QuestionHelper.isEdit || QuestionHelper.modified || answer !== QuestionHelper.current.answer) {
            ...
            if (QuestionHelper.isJudge) {
                data = "id=" + (QuestionHelper.current ? QuestionHelper.current.id : "-1") + "&title=" + values[0] +
                    "&score=" + values[1] + "&answer=" + answer + "&type=" + QuestionHelper.questionType;
            } else {
                //参数串
                data = "id=" + (QuestionHelper.current ? QuestionHelper.current.id : "-1") + "&title=" + values[0] + "&optiona=" + values[1] +
                    "&optionb=" + values[2] + "&optionc=" + values[3] + "&optiond=" + values[4] + "&score=" + values[5]
    ...
    showRate: function (button) {
        ...
        var id = $(button).parents("tr").find("td:first").html();
        //发送ajax请求
        $.post(getRootPath()+"/teacher/question/rate/" + id, null, function (data) {
            if (data.code != 200) {
                Tips.showError(data.msg);
            } else {
                $("#rate-window").modal("show");
                $("#div_info").html(data.data);
            }
        }, "json");
    },
    //关闭正答率窗体
    closeRate: function () {
        $("#rate-window").modal("hide");
    }
};